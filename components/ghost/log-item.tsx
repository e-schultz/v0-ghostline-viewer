...
function LogItemComponent({ log, index, onToggle, onViewOriginal, onCopy }: LogItemProps) {
  ...
  const isAudio = log.type === "audio" && log.audioUrl
      ...
      >
        ...
        {isAudio && (
          <div className="mr-2 flex-shrink-0">
            <AudioPlayer
              src={log.audioUrl!}
              id={`audio-${log.id}-${index}`}
              compact={true}
              duration={log.audioDuration}
            />
          </div>
        )}
        ...
        >
          {isAudio ? (
            <div className="space-y-3">
              <div className="text-sm mb-2 break-words">{log.audioDescription}</div>
              <AudioPlayer src={log.audioUrl!} id={`audio-${log.id}-${index}-full`} duration={log.audioDuration} />
            </div>
          ) : (
          ...
          <div className="flex flex-wrap gap-2 mt-2">
            ...
            {isAudio && (
              <button
                className="text-xs flex items-center hover:bg-pink-500/20 p-1 rounded"
                onClick={() => onCopy(log.audioDescription || "")}
                aria-label={`Copy audio description for ${log.title}`}
              >
                <Copy size={14} className="mr-1 flex-shrink-0" aria-hidden="true" />
                Copy Description
              </button>
